...
//							System.out.println(volumeObj);
							...
							if(!("default-token-e0d8c".equals(volName))){
								JsonObject volumeClaim = volumeObj.getAsJsonObject("persistentVolumeClaim");
								String claimName = volumeClaim.get("claimName").getAsString();
								if(!(name.equals(claimName))){
									System.out.println("name:" + volName +",claimName: " + claimName);
									name = claimName;
								}else{
									lr.error_message("volumes is common");
									isSuccess = false;
								}
							}
