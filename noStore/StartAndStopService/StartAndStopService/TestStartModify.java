...
public class TestStartModify implements TestCaseValidater{
		...
		if("".equals(ret)){
			...
			try{
				...
				if(jsonObj.get("ret").getAsBoolean() == false){
					...
					if("stop".equals(jsData.get("totalStatue").getAsString())){
						System.out.println("starting");
					}else{
						isSuccess = false;
						System.out.println("state is not stop.cant running");
					}
					return isSuccess;
				}
			}catch(Exception err){
